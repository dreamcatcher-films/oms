...
const readFileWithProgress = (file: File): Promise<ArrayBuffer> => {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            logToMain('log', `[Parser Worker] Finished reading file ${file.name} into buffer.`);
            resolve(event.target?.result as ArrayBuffer);
        };

        reader.onerror = () => {
            logToMain('error', `[Parser Worker] Error reading file ${file.name}.`);
            reject(reader.error);
        };
        ...
        reader.onprogress = (event) => {
            if (event.lengthComputable) {
                const percentage = (event.loaded / event.total) * 100;
                 logToMain('log', `[Parser Worker] Reading file ${file.name}: ${percentage.toFixed(2)}%`);
                postMessage({
                    type: 'progress',
...
self.onmessage = async (e: MessageEvent<ShcParsingWorkerRequest>) => {
    ...
    logToMain('log', `[Parser Worker] Received file for processing. Type: ${dataType}, Name: ${file.name}, Size: ${file.size} bytes`);
    ...
    if (dataType === 'shc') {
        ...
        logToMain('log', `[Parser Worker] Starting CSV stream parsing for ${file.name}`);
        ...
        Papa.parse(file, {
            ...
            chunk: (results, parser) => {
                ...
                logToMain('log', `[Parser Worker] Chunk #${chunkCount} processed: ${chunkTotalRows} rows read, ${chunkAcceptedRows} rows accepted (Status=8, Unit=C).`);

                if (batch.length >= BATCH_SIZE_FOR_POSTING) {
                    logToMain('log', `[Parser Worker] Posting batch of ${batch.length} rows to main thread.`);
            ...
            complete: () => {
                if (batch.length > 0) {
                    logToMain('log', `[Parser Worker] Posting final batch of ${batch.length} rows to main thread.`);
                    postMessage({ type: 'data', payload: batch } as ShcParsingWorkerMessage);
                }
                
                logToMain('log', `[Parser Worker] CSV stream parsing complete for ${file.name}. Total rows read: ${totalRows}, Total rows accepted: ${acceptedRows}`);
                postMessage({ type: 'complete', payload: { totalRows: acceptedRows } } as ShcParsingWorkerMessage);
            },
            error: (error) => {
                logToMain('error', 'Error in PapaParse streaming:', error);
                postMessage({
                    type: 'error',
        ...
        try {
            logToMain('log', `[Parser Worker] Starting Excel file processing for ${dataType}`);
            ...
            logToMain('log', `[Parser Worker] Parsing Excel buffer for ${dataType}...`);
            const parsedData = parseShcFile(dataType, buffer);
            const totalRows = parsedData.length;
            logToMain('log', `[Parser Worker] Excel parsing complete. Found ${totalRows} valid rows.`);
            ...
            for (let i = 0; i < totalRows; i += BATCH_SIZE_FOR_POSTING) {
                ...
                logToMain('log', `[Parser Worker] Posting batch ${i / BATCH_SIZE_FOR_POSTING + 1} of ${Math.ceil(totalRows / BATCH_SIZE_FOR_POSTING)} with ${batch.length} rows.`);
                postMessage({ type: 'data', payload: batch } as ShcParsingWorkerMessage);
            }

            logToMain('log', `[Parser Worker] All data sent to main thread for ${dataType}.`);
            postMessage({ type: 'complete', payload: { totalRows } } as ShcParsingWorkerMessage);

        } catch (error) {
            logToMain('error', `Error in SHC parsing worker (${dataType}):`, error);
            postMessage({
                type: 'error',
